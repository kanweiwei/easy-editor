...
/* eslint-disable */
var transUnitToPx = function transUnitToPx(val) {
  if (!/(pt|cm)/.test(val)) {
    return val;
  }
  ...
  if (unit) {
    switch (unit) {
      case "cm":
        val = _parseFloat(val) * 25;
        break;

      case "pt":
        val = Math.round(_parseFloat(val) * 96 / 72);
...
var filterWord = function () {
  ...
  function filterPasteWord(str) {
    return str // 列表项
    .replace(/(\d{1,}.)(&nbsp;){8,}/g, "$1&nbsp;").replace(/[\t\r\n]+/g, " ").replace(/<!--[\s\S]*?-->/gi, "") //转换图片
    ...
    // .replace(/<\/?div[^>]*>/g, "")
    // .replace(/<(\/?)p([^>]*)>/g, "<$1div$2>")
    .replace(/<head[^>]*>.*?<\/head>/gi, "").replace(/<(html|body)[^>]*>(.*?)<\/\1>/gi, "$2").replace(/<(html|body)[^>]*>(.*?)<\/\1>/gi, "$2") //去掉多余的属性
    ...
    .replace(/\s+(lang|align)\s*=\s*(['"]?)([\w-]+)\2/gi, function (str, name, marks, val) {
      //保留list的标示
      ...
      for (var i = 0, v; v = s[i]; i++) {
        ...
        if (parts.length == 2) {
          ...
          switch (name) {
            ...
            case "mso-margin-left-alt": //ie下会出现挤到一起的情况
