...
function getTdCount(node) {
    var trs = node.filterDescendants(function (n) { return n.type === "table-row"; });
    var res = 0;
    trs.forEach(function (tr) {
        var tds = tr.filterDescendants(function (n) { return n.type === "table-cell"; });
        if (tds.size > res) {
            res = tds.size;
        }
    });
...
var Vline = /** @class */ (function (_super) {
    ...
    Vline.prototype.render = function () {
        ...
        return (<div className="slate-sheet-container-vline" style={{ left: left + "px" }} onMouseDown={this.startResize}/>);
    };
    return Vline;
}(React.Component));
var SlateTable = /** @class */ (function (_super) {
    ...
    function SlateTable(props) {
            ...
            return function (change) {
                ...
                if (target) {
                    ...
                    change.setNodeByKey(target.key, {
                        data: __assign(__assign({}, target.data.toJS()), { style: style }),
                    });
                }
            };
        };
        _this.updateWidth = function (left, i) {
            var _a = _this.state, maxTdCount = _a.maxTdCount, vlines = _a.vlines;
            var _b = _this.props, node = _b.node, editor = _b.editor;
            ...
            var targetTrs = node.filterDescendants(function (n) { return n.type === "table-row" && n.nodes.size === maxTdCount; });
        ...
        _this.renderVlines = function () {
            ...
            return range(vlines.length).map(function (n, i) {
                return (<Vline left={vlines[i].left} onUpdate={function (left) { return _this.updateWidth(left, i); }} key={i}/>);
            });
        };
        _this.updateVlinesPosition = function () {
            ...
            var targetTr = node.findDescendant(function (n) { return n.type === "table-row" && n.nodes.size === maxTdCount; });
            if (!targetTr) {
                return;
            }
        ...
        _this.changeTableBorder = function () {
            ...
            if (isFocused) {
                ...
                range(tds_2.length).forEach(function (n, i) {
                    tds_2[i].style.border = "none";
                });
            }
        };
        var maxTdCount = getTdCount(props.node);
        var vlines = range(maxTdCount).map(function () {
            return {
                left: 0,
            };
        });
    ...
    SlateTable.prototype.componentDidMount = function () {
        ...
        if (root.border ||
            root.style.borderWidth ||
            slice(tds).some(function (td) { return td.style.borderWidth !== null; })) {
            this.hasBorder = true;
        }
        else {
    ...
    SlateTable.prototype.render = function () {
        var _a = this.props, attributes = _a.attributes, children = _a.children, node = _a.node;
        ...
        return (<div className="slate-sheet-container" {...attributes}>
        {this.renderVlines()}
